...
import { getRegisteredUsers, useAuth } from '../context/AuthContext';
...
export default function TrainingReport() {
  ...
  const { getProgressForUser } = useTrainingProgress();
  const users = useMemo(() => getRegisteredUsers(), []);

  const [selectedUserId, setSelectedUserId] = useState(user?.id || '');
  const effectiveUserId = isAdmin ? selectedUserId || user?.id || '' : user?.id || '';
  const targetUser = users.find((entry) => entry.id === effectiveUserId) || user || null;
  const progress = effectiveUserId ? getProgressForUser(effectiveUserId) : null;
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl md:text-3xl font-bold text-gray-900 mb-2">📈 Relatório de Treinamento</h1>
        <p className="text-gray-500">Acompanhe evolução por operador, tentativas de quiz e certificados emitidos.</p>
      </div>

      {isAdmin && (
        <div className="bg-white rounded-xl p-4 border border-gray-100 shadow-sm">
          <label className="text-sm font-medium text-gray-700 block mb-2">Selecionar operador</label>
          <select
            value={effectiveUserId}
            onChange={(e) => setSelectedUserId(e.target.value)}
            className="w-full md:w-96 px-3 py-2.5 rounded-lg border border-gray-200 text-sm"
          >
            {users.map((entry) => (
      ...
      {targetUser && progress ? (
        <>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
            <StatCard title="Operador" value={targetUser.name} emoji="👤" />
            <StatCard title="Módulos" value={`${progress.completedModuleIds.length}/${trainingModules.length}`} emoji="📚" />
            ...
            {progress.certificates.length > 0 ? (
              <div className="space-y-2">
                {progress.certificates.map((cert) => (
                  <div key={cert.id} className="bg-green-50 border border-green-200 rounded-lg p-3 text-sm flex items-center justify-between gap-3">
                    <div>
                    ...
                    <div className="flex items-center gap-2">
                      <span className={`text-xs px-2 py-1 rounded-full bg-white font-medium border ${
                        isCertificateExpired(cert) ? 'border-red-300 text-red-700' : 'border-green-300 text-green-700'
                      }`}>
                        {getCertificateStatusLabel(cert)}
                      </span>
                      {targetUser && (
                        <button
                          onClick={() => printCertificateAsPdf({ certificate: cert, operatorName: targetUser.name, operatorRole: targetUser.role })}
        ...
        <p className="text-sm text-gray-500">Não foi possível carregar o relatório.</p>
      )}
    </div>
  );
}
